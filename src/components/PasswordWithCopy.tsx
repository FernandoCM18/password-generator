...
export const PasswordWithCopy = ({text, copyClipboard}: Props) => {
  ...
  return (
    <div className="flex justify-between items-center bg-gray-100 dark:bg-slate-800 p-5">
      <p className={`text-gray-700 dark:text-gray-500 ${text.length > 0 && 'dark:text-slate-300'} break-all max-w-[200px] md:max-w-full`}>{text || 'Genera una contraseña'}</p>
      <div onClick={handleCopyClipboard} className="p-1 rounded-md lg:hover:bg-gray-200 dark:lg:hover:bg-slate-700 focus:bg-transparent">
        <svg className={`${text ? 'fill-blue-600' : 'fill-gray-700 dark:fill-gray-500'} ${text.length === 0 && 'cursor-not-allowed'}`} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
